...
class LongTextQueue:
    ...
    def __init__(self):
        ...
        try:
            ...
            # 初始化队列
            self.queue = rq.Queue(
                name=config.LONG_TEXT_CONTENT_PROCESS_QUEUE, 
    ...
    def cleanup_jobs(self) -> Dict[str, int]:
        ...
        try:
            # 清理失败任务
            failed_count = 0
            failed_cutoff = datetime.now() - timedelta(seconds=config.FAILED_JOBS_RETENTION)
            for job_id in self.failed_registry.get_job_ids():
                job = Job.fetch(job_id, connection=self.redis)
            ...
            # 清理完成任务
            finished_count = 0
            finished_cutoff = datetime.now() - timedelta(seconds=config.FINISHED_JOBS_RETENTION)
