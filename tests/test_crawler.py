...
from weibo_favorites.crawler.auth import load_cookies, create_session
...
def test_unit_crawler_state():
    ...
    test_state = {
        "last_id": "123456",
        "last_crawl_time": "2024-12-29 11:30:00"
    }
...
def test_unit_crawl_favorites_basic(mock_queue, mock_session):
    ...
    with patch('builtins.open', m), \
         patch('weibo_favorites.crawler.crawler.create_session', return_value=mock_session), \
         patch('weibo_favorites.crawler.crawler.save_weibo') as mock_save_weibo:
        
        # 执行爬取
        result = crawl_favorites(cookies=[{"cookie": "test"}], ltp_queue=mock_queue, page_number=1)
